...
<style>
    ...
    .rm-item { border: 1px solid #e2e8f0; border-radius: 22px; background: #ffffff; padding: 20px; box-shadow: 0 8px 20px rgba(15, 23, 42, .06); display: grid; grid-template-columns: 170px 1fr; gap: 20px; align-items: center; }

    .rm-date-box { background: linear-gradient(135deg, #8b5cf6, #6366f1); color: white; border-radius: 20px; padding: 18px; text-align: center; }
...
<div class="rm-page">
    <div class="rm-main-card">
        <div class="rm-header">
            <div>
                <h1 class="rm-title">Resep Obat</h1>
                <p class="rm-subtitle">Daftar resep obat yang pernah diberikan kepada pasien.</p>
            </div>
        </div>

        <form method="GET" action="{{ route('tenaga-medis.resep.index') }}" class="rm-filter">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama pasien...">
            <button type="submit" class="btn-filter">Cari</button>
            <a href="{{ route('tenaga-medis.resep.index') }}" class="btn-reset">Reset</a>
        </form>
            ...
            @forelse($rekamMedis as $item)
                ...
                <div class="rm-item">
                    ...
                    <div>
                        ...
                        <div class="rm-info">
                            <div class="rm-info-label">Resep Obat</div>
                            <div class="rm-info-value">{{ $item->resep_obat }}</div>
                        </div>
                    </div>
                </div>
            @empty
                <div class="empty-box">Belum ada resep obat yang tercatat.</div>
            @endforelse
        </div>
    </div>
</div>
